...
// Define a macro to convert the version numbers to a string
#define STRINGIFY(x) #x
#define TOSTRING(x) STRINGIFY(x)
#include "esp_task_wdt.h"

#define APP_VERSION_STRING TOSTRING(APP_VERSION_MAJOR) "." TOSTRING(APP_VERSION_MINOR) "." TOSTRING(APP_VERSION_BUILD)
#define OS_VERSION_STRING TOSTRING(OS_VERSION)
#define OS_NAME_STRING TOSTRING(OS_NAME)
...
// Buffer sizes for MQTT topics and JSON messages
#define TOPIC_BUFFER_SIZE 128
#define JSON_BUFFER_SIZE 1024
#define LOG_BUFFER_SIZE 8096
#define MAX_PAYLOAD_SIZE 4096

// Buffers for MQTT topics, JSON messages, and logs
static char ShadowTopic[TOPIC_BUFFER_SIZE];
static char jsonMessage[JSON_BUFFER_SIZE];
static char log_buffer[LOG_BUFFER_SIZE];
static int log_buffer_index = 0;

int custom_log_function(const char *fmt, va_list args)
{
    int len = vsnprintf(log_buffer + log_buffer_index, LOG_BUFFER_SIZE - log_buffer_index, fmt, args);
    if (len > 0)
    {
        log_buffer_index += len;
        if (log_buffer_index >= LOG_BUFFER_SIZE - 1)
        {
            log_buffer_index = 0;
        }
    }
...
void publishLogChunk(const char *randomId, const char *macAddress, const char *chunk, size_t chunkLen)
{
    const char *baseTopic = "$aws/things/";
    snprintf(ShadowTopic, sizeof(ShadowTopic), "%s%s/logs", baseTopic, macAddress);

    cJSON *logRoot = cJSON_CreateObject();
    cJSON_AddStringToObject(logRoot, "id", randomId);
    cJSON_AddStringToObject(logRoot, "log_data", chunk);
    cJSON_AddNumberToObject(logRoot, "timestamp", (long)time(NULL));
    cJSON_AddStringToObject(logRoot, "mac_address", macAddress);

    char *logMessage = cJSON_PrintUnformatted(logRoot);
    if (logMessage == NULL)
    {
    ...
    uint16_t topicLen = (uint16_t)strlen(ShadowTopic);
    uint32_t msgSize = (uint32_t)strlen(logMessage);
    uint8_t qos = 0; // QoS 0

    // Publish the log chunk
    mqttPublish(ShadowTopic, topicLen, logMessage, msgSize, qos);
...
void trigger_alarm_mqtt(const char *macAddress)
{
    srand(time(NULL));
    int length = 10;
    char id[length + 1];
    ...
    {
        const char *baseTopic = "$aws/things/";
        snprintf(ShadowTopic, sizeof(ShadowTopic), "%s%s/errors", baseTopic, macAddress);

        cJSON *logRoot = cJSON_CreateObject();
        cJSON_AddStringToObject(logRoot, "id", id);
        cJSON_AddNumberToObject(logRoot, "timestamp", (long)time(NULL));
        cJSON_AddStringToObject(logRoot, "mac_address", macAddress);
        cJSON_AddStringToObject(logRoot, "Device Failed", macAddress);
...
// Function to send logs via MQTT
void send_logs_via_mqtt(const char *macAddress)
{
    srand(time(NULL));
    int length = 10;
    char id[length + 1];
    ...
    generateRandomID(id, length);
    // Check if the MQTT session is established
    if (mqttSessionEstablished == true)
    {
        size_t offset = 0;

        while (offset < log_buffer_index)
        {
            size_t chunkLen = (log_buffer_index - offset > MAX_PAYLOAD_SIZE) ? MAX_PAYLOAD_SIZE : log_buffer_index - offset;
            char chunk[MAX_PAYLOAD_SIZE + 1];
            strncpy(chunk, log_buffer + offset, chunkLen);
            chunk[chunkLen] = '\0';

            publishLogChunk(id, macAddress, chunk, chunkLen);
            offset += chunkLen;
        }
...
void send_config_mqtt(const char *macAddress)
{
    if (mqttSessionEstablished != true)
    {
        int ret = establishConnection();
    ...
    if (mqttSessionEstablished == true)
    {
        ...
        snprintf(ShadowTopic, sizeof(ShadowTopic), "%s%s/config", baseTopic, macAddress);
        cJSON *root = cJSON_CreateObject();
        cJSON_AddStringToObject(root, "message", "Device wakes");
        cJSON_AddNumberToObject(root, "timestamp", (long)time(NULL));
        cJSON_AddStringToObject(root, "OS", OS_NAME_STRING);
        cJSON_AddStringToObject(root, "OS_version", OS_VERSION_STRING);
        cJSON_AddStringToObject(root, "app_version", APP_VERSION_STRING);
        cJSON_AddStringToObject(root, "mac_address", macAddress);
        ...
        uint8_t qos = 0; // QoS 0
        mqttPublish(ShadowTopic, topicLen, jsonMessage, msgSize, qos);
...
void subscribe_test()
{
    ...
    if (mqttSessionEstablished == true)
    {
        const char *topic = "test/subscribe";
...
void app_main()
{
    ...
    */
    ...
    // aaaaa use for quick search
    send_config_mqtt(macAddress);
    subscribe_test();

    // test error
    // ESP_LOGE(TAG, "TEST ERROR SEND SNS");
    // trigger_alarm_mqtt(macAddress);
    // send_logs_via_mqtt(macAddress);
